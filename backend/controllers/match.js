...
exports.getPossibleMatches = async (req, res, next) => {
  try {
    ...
    let filter = {
      _id: { $nin: [req.params.id, ...matchedId] },
    };

    if (user.preference !== "Any") filter.sex = user.preference;
...
exports.addMatch = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate(sender, {
      $inc: { dailySwipeRight: 1, allTimeSwipeRight: 1 },
    });

    //check if they are matched

    const recordExists = await Match.findOne({
      sender: receiver,
      receiver: sender,
    });
    if (recordExists) {
      ...
      await User.findByIdAndUpdate(sender, {
        $inc: { dailyMatch: 1, allTimeMatch: 1, experience: 69 },
      });
